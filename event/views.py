...
class EventPageServe(View):
    ...
    def get(self, request, *args, **kwargs):
        if not request.site:
            raise Http404
        if request.resolver_match.url_name == 'event_page_serve_slug':
            # Splitting the request path and obtaining the path_components
            # this way allows you to place the events listing at any level.
            # Example:
            # splited_path =  ['fr', 'events', '2016', '06', '23', 'event-entry']
            # slicing this way you obtain:
            # path_components =  ['fr', 'events', 'event-entry']
